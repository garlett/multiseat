...
# weston 10 and drm lease manager builder on archlinux.
...
then
    ...
    echo "source+=(\"https://gerrit.automotivelinux.org/gerrit/gitweb?p=AGL/meta-agl-devel.git;a=blob_plain;f=meta-agl-drm-lease/recipes-graphics/weston/weston/\"{0001-compositor-do-not-request-repaint-in-output_enable,0004-launcher-direct-handle-seat0-without-VTs}\".patch\")" >> PKGBUILD
...
sudo -uuserdw makepkg -f -s --skippgpcheck || exit 150 # TODO: insert keys on user and remove skippgpcheck
...
weston --drm-lease=card0-DVI-I-1 --log=log_weston.log
cat log_weston.log
