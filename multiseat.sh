...
keyboards=('/sys/class/input/input5' '/sys/class/input/input4' )
mouses=('/sys/class/input/input2' '/sys/class/input/input3' )
...
case "$1" in 
    ...
    "-c") # create config
	unset skeyboards smouses sleases 
	...
	sed -i "s/keyboards=(.*)$/keyboards=($(sed -e 's/[&\\/]/\\&/g; s/$/\\/' -e '$s/\\$//' <<<"$skeyboards"))/" $0 || exit 133
	sed -i "s/mouses=(.*)$/mouses=($(sed -e 's/[&\\/]/\\&/g; s/$/\\/' -e '$s/\\$//' <<<"$smouses"))/" $0 || exit 134
	sed -i "s/leases=(.*)$/leases=($(sed -e 's/[&\\/]/\\&/g; s/$/\\/' -e '$s/\\$//' <<<"$sleases"))/" $0 || exit 135
	#vi $0
	...
	do
		...
		loginctl attach seat_$lease ${keyboards[$seat_pos]} ${mouses[$seat_pos]} || exit 160
		seat_pos=$(($seat_pos + 1))
...
#	chvt 1
    ...
    *) # -a auto  -g clone  -l link  -d start dlm
